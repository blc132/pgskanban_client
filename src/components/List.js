...
export default class List extends Component {
    ...
    renderCards = () => {
        if (this.props.cards) {
            return this.state.cards.map((card) => <Card key={card.id} cardId={card.id} cardName={card.name} onDelete={this.deleteCard} />)
        }
    };
    ...
    saveListName = () => {
        axios.put(BASE_URL + '/list/' + this.props.listId,
            { name: this.state.name, boardId: this.props.boardId }
        ).then(() => {
